...
void lv_draw_wifi_tips() {
  ...
  // mks_printer.user_src = lv_screen_create(WIFI_TIPS_UI, "");
  lv_screen_distribution(WIFI_TIPS_UI);
  wifi_name = lv_label_create(mks_printer.user_src, (const char *)wifi_list.wifiName[wifi_list.nameIndex]);
  lv_obj_align(wifi_name, nullptr, LV_ALIGN_CENTER, 0, -20);

  text_tips = lv_label_create_empty(mks_printer.user_src);
  if (wifi_tips_type == TIPS_TYPE_JOINING) {
    lv_label_set_text(text_tips, tips_menu.joining);
    lv_label_set_style(text_tips,LV_LABEL_STYLE_MAIN,&test_font_16);
...
void lv_clear_wifi_tips() { lv_obj_clean(mks_printer.user_src); }
