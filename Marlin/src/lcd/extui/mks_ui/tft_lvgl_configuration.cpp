...
#include <SPI.h>
  ...
  void init_DengXian_16();
  void init_DengXian_12();
  void init_DengXian_8();
  void init_DengXian_20();
#endif
...
void SysTick_Callback() {
  ...
  print_time_count();
  #if ENABLED(MKS_WIFI_MODULE)
    if(wifi_connect_data.start)
    {
      wifi_connect_data.time++;
    }
...
void tft_lvgl_init() {
  ...
  lv_disp_buf_init(&disp_buf, bmp_public_buf, nullptr, LV_HOR_RES_MAX * 20); // Initialize the display buffer
  ...
  TERN_(HAS_SPI_FLASH_FONT, init_DengXian_16());
  TERN_(HAS_SPI_FLASH_FONT, init_DengXian_12());
  TERN_(HAS_SPI_FLASH_FONT, init_DengXian_8());
  TERN_(HAS_SPI_FLASH_FONT, init_DengXian_20());

  tft_style_init();
  filament_pin_setup();
  lv_encoder_pin_init();

  mks_printer.user_src = lv_obj_create(nullptr, nullptr);
  lv_obj_set_style(mks_printer.user_src, &tft_style_scr);
  lv_scr_load(mks_printer.user_src);
  ...
  bool ready = true;
    ...
    if (recovery.valid()) {
      ...
      #endif
      // z_offset_add = gCfgItems.babystep_data;
      lv_draw_dialog(DIALOG_TYPE_RECOVERY_TIPS);
      // lv_draw_printing();
      // draw_rec_dialog();
    }
  #endif
...
bool my_touchpad_read(lv_indev_drv_t * indev_driver, lv_indev_data_t * data) {
  ...
  if (get_point(&last_x, &last_y)) {
    ...
    #elif TFT_ROTATION == TFT_ROTATE_270
      if(last_x > TFT_HEIGHT) last_x = TFT_HEIGHT;
      if(last_y > TFT_WIDTH) last_y = TFT_WIDTH;

      data->point.x = last_y;
      data->point.y = TFT_HEIGHT - last_x;
      
      // sprintf_P(buff, PSTR(" %d %d"), data->point.x,data->point.y);
      // SERIAL_ECHO_MSG("hello",((const char *)buff),"\r\n");
    #else
      data->point.x = last_x;
      data->point.y = last_y;
    #endif
