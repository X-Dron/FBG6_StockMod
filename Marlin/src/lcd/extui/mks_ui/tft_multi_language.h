...
typedef struct move_menu_disp {
  ...
  const char *zoffset;
  const char *layout_stop;

  const char *back;
} move_menu_def;
extern move_menu_def                move_menu;

typedef struct layout_menu_disp {
  const char *title;
} layout_menu_def;

extern layout_menu_def layout_menu;
...
typedef struct dialog_menu_disp {
  ...
  const char *complete_btn;
  const char *print_again;
  const char *recovery_continue_btn;
  const char *recovery_back_btn;
  const char *recovery_text;
  const char *emergency_text1;
  const char *emergency_text2;
  const char *emergency_text3;
  const char *emergency_text4;
  const char *no_filament_text1;
  const char *no_filament_text2;
  const char *no_filament_text3;
...
typedef struct voice_menu_disp {
  const char *title;
  const char *btn_sound;
  const char *endstop_sound;
  const char *print_tone_sound;
  const char *no_filament_tone_sound;
}voice_menu_def;
extern voice_menu_def        voice_menu;

typedef struct advances_menu_disp {
  const char *title;
  const char *floodlight;
  const char *fan5051;
  const char *flame_alarm;
}advances_menu_def;
extern advances_menu_def advances_menu;

typedef struct endstop_menu_disp {
  const char *title;
  const char *x_endstop;
  const char *y_endstop;
  const char *z_endstop;
  const char *det_break;
  const char *flame_sensor;
  const char *open;
  const char *trigger;

}endstop_menu_def ;
extern endstop_menu_def         endstop_menu;


typedef struct tempsetting_menu_disp {
  const char *title;
  const char *pla;
  const char *abs;
  const char *ex1;
  const char *ex2;
  const char *bed;
}tempsetting_menu_def;
extern tempsetting_menu_def        tempsetting_menu;
...
/*****************************************/
//
#define TEXT_TEMP_VALUE          "%d°C"
#define TEXT_VALUE          "%d/%d"
#define TEXT_VALUE_TARGET   "%d°C/%d°C"
